...
the pointers to point at new indices based on certain conditions. A 
common variation of this technique is to point one variable at the 
beginning of one list/string and a second pointer at the beginning of a
 second list/string, then increment each pointer conditionally to solve
   a problem.
...
def merge_sorted_lists(lst1, lst2):
    ...
    while first < len(lst1) and second < len(lst2):
        if lst1[first] < lst2[second]:
            merge.append(lst1[first])
            first +=1
        else:
            merge.append(lst2[second])
            second +=1
    
    merge.extend(lst1[first:])
    merge.extend(lst2[second:])
    return merge
